...
- [x] ~~HQ Admin has no login.~~ **DONE — replaced outright, not run alongside the old stopgap.** `HQ_ADMIN_KEY`, `HqAdminKeyGuard`, its env var and its production-refusal check are all REMOVED. Every `/admin/*` controller (Staff, Orders, Menu, Customers) now requires a real HQ session (`HqAdminSessionGuard`) from `POST /auth/hq/login` — a real password (not a PIN; see `StaffPasswordSchema`'s doc comment for why), looked up by phone, checked with the same `crypto.scrypt` primitive as POS's PIN. The browser holds an httpOnly session cookie (`apps/hey-food-hq/src/lib/session.ts`); the backend session itself is the same `StaffSession` mechanism POS uses (`outletId` is `NULL` for an HQ session — never bound to one outlet). `outlet_staff` is rejected outright (403); `hq_admin` gets full access. What that closes and what is still open:
  - **Every `businessId` that used to be a client-supplied field is now session-derived** — removed from every admin request/query contract. This also closed several pre-existing, previously-unscoped gaps found along the way, corrected here after under-reporting them the first time: `GET /admin/orders/:id`, `POST /admin/orders/:id/cancel`, `GET /admin/products/:id`, `PATCH /admin/products/:id`, the override endpoint, **and** `GET/PATCH /admin/staff/:id`, `.../reset-pin`, `.../reset-password`, `.../deactivate`, `.../reactivate` (all nine via `AdminOrdersService`/`AdminMenuService`/`AdminStaffService`'s shared by-id lookups) had NO business-ownership check at all before this pass (any business's session could read, write, or take over another's by id alone); all nine are scoped now. A tenth instance, `GET /admin/customers/:id`, was missed in the same pass and fixed in a follow-up commit after a review question surfaced it — see docs/STATUS.md's "Follow-up fix" row for the full write-up, including a genuine red/green demonstration of the leak.
