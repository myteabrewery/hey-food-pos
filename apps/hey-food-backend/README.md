...
- [ ] **Audit trail: who changed an order or menu item — the gap has SHRUNK, but is not closed.** A status change or cancel stores WHEN (`preparing_at`, `ready_at`, `collected_at`/`completed_at`, `cancelled_at`) and, for a cancel, WHY (`cancel_reason`, `cancel_reason_detail`); `orders.cancel_source` and `menu_change_events.source` record WHERE (`hq` vs `pos`) — but **still not WHO**, on either table. This used to be blocked on "no authentication exists to attribute a change to"; **that is no longer true.** Real, authenticated staff identity (`staffId`, from the session `StaffSessionGuard` attaches to every POS request) now exists at the exact point every POS write happens — `PosOrdersService.advance`/`cancel`, `PosMenuService.setAvailability` all receive a `StaffSessionContext` with a real `staffId` already. **It is simply not persisted onto `orders` or `menu_change_events` yet.** Deliberately deferred as its own follow-up, not bundled into the login pass: retrofitting a `staffId` column onto each needs its own migration and its own re-verification of already-heavily-tested services, and `orders`' automatic transitions (`paid→received` on sync, `collected→completed`) genuinely have no human actor, so any FK has to be nullable. The natural next pass, now that the hard dependency (real login) is satisfied.
