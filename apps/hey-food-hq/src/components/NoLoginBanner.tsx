 ...
 * (with Menu Management) can now change prices for the whole business, so this
...
export function NoLoginBanner(): ReactElement {
  return (
    <div role="alert" className="border-b-2 border-danger-solid bg-danger-tint px-8 py-2 text-hq-caption font-bold text-danger-solid">
      ⛔ NO LOGIN — anyone who can open this app can change ANY price and availability for the whole business. It must only
      ever run on localhost; never expose, tunnel or deploy it until real HQ authentication exists.
    </div>
  );
}
