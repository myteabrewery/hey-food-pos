 ...
 * record stays, so history and the audit trail's actor column still
 * reference it; only login is blocked — immediately, for both real POS PIN
 * login and real HQ password login). Deactivating asks first, since it can
 * lock someone out; the backend also refuses to deactivate the business's
 * LAST active HQ Admin — this button just surfaces that message if it happens.
 */
    ...
    if (confirming) {
      return (
        <div className="rounded-md border border-danger-solid bg-danger-tint p-3">
          <p className="text-hq-body font-semibold text-danger-solid">Deactivate this account? They will not be able to log in — to POS or HQ — the moment this saves. The record itself is kept.</p>
